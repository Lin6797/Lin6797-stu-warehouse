...
minHeap::minHeap(int n){ //构造函数 
	Num=0; Capacity=n; //初始化 
	tHeap=new TreeNode*[n]; //开辟空间 
	eHeap=new int[n];
}

bool minHeap::IsEmpty(){//判断堆是否为空 
	if(Num==0) return true;
	else return false;
}

bool minHeap::IsFull(){//判断堆是否为满 
	if(Num==Capacity) return true;
	else return false;
}

bool minHeap::SiftUp(int start){//上滑调整堆（默认start=Num-1） 
	int j=start, i=(j-1)/2; //j为起始点，i为起始点的父母点
	TreeNode* ttemp=tHeap[start]; int etemp=eHeap[start]; //两种类型的中间量 
	while(j>0){
...
bool minHeap::SiftDown(int start, int end){//下滑调整堆（默认start=0,end=Num-1） 
	int i=start, j=(2*i)+1; //i为起点,j为起点左下角的结点 
	TreeNode* ttemp=tHeap[i]; int etemp=eHeap[i]; //两种类型的起始量 
	while(j<=end){
...
bool minHeap::Insert(TreeNode* t_data, int e_data){ //插入 
	if(IsFull()){
		cout<<"堆已满，不可插入"<<endl;
		return false;
	}
...
bool minHeap::Build(TreeNode* t_data, int e_data){ //建表 
	Insert(t_data, e_data);
	return false;
}

TreeNode* minHeap::Delete(){//删除并返回 
	if(IsEmpty()){
